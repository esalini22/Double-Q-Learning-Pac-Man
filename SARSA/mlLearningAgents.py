...
class QLearnAgent(Agent):
    ...
    def getAction(self, state, debug_mode=True):
        ...
        self.next_action = self.epsilon_greedy(state, legal)
    ...
    def update_Q_value(self, state, final_step=False):
        ...
        next_Q = 0.0
        if not final_step:
            next_Q = self.Q_values[state][self.next_action]
        # update Q(s, a)
        self.Q_values[self.prev_state][self.prev_action] += (self.alpha * (reward + self.gamma * next_Q - self.Q_values[self.prev_state][self.prev_action]))
