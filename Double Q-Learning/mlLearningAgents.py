...
class QLearnAgent(Agent):
    ...
    def getAction(self, state, debug_mode=True):
        ...
        # initialize Q-value
        if state not in self.Qa_values or state not in self.Qb_values:
            self.initialize_Q_values(state, legal)

        self.prob = random.choice([0,1]) #elige Q^A o Q^B con probabilidad 0.5
    ...
    def initialize_Q_values(self, state, legal):
        self.Qa_values[state] = dict()
        self.Qb_values[state] = dict()
        for action in legal:
            if action not in self.Qa_values[state]:
                self.Qa_values[state][action] = 0
            if action not in self.Qb_values[state]:
                self.Qb_values[state][action] = 0
    ...
    def update_Q_value(self, state, final_step=False):
        ...
        if not final_step:
            if self.prob>0.5:
                max_Q_action = max(self.Qa_values[state],key=self.Qa_values[state].get)
                #print(type(max_Q_action))
                max_Q_value = self.Qb_values[state][max_Q_action]
            else:
                max_Q_action = max(self.Qb_values[state],key=self.Qb_values[state].get)
                #print(type(max_Q_action))
                max_Q_value = self.Qa_values[state][max_Q_action]
        # update Q(s, a)
        if self.prob>0.5:
            self.Qa_values[self.prev_state][self.prev_action] += (self.alpha * (reward + self.gamma * max_Q_value - self.Qa_values[self.prev_state][self.prev_action]))
        else:
            self.Qb_values[self.prev_state][self.prev_action] += (self.alpha * (reward + self.gamma * max_Q_value - self.Qb_values[self.prev_state][self.prev_action]))
    ...
    def epsilon_greedy(self, state, legal):
        ...
        for action in legal:
            if max_Q_action == None:
                max_Q_action = action
            if self.Qa_values[state][action] + self.Qb_values[state][action] > self.Qa_values[state][max_Q_action] + self.Qb_values[state][max_Q_action]:
                max_Q_action = action
        return max_Q_action
    ...
    def final(self, state, debug_mode=True):
        ...
        #disminuimos epsilon a medida que aprende
